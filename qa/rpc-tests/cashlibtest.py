...
class MyTest(BitcoinTestFramework):
    ...
    def runScriptMachineTests(self):
        ...
        worked = sm.eval(
            CScript([OP_1, OP_0, OP_5, OP_6, OP_TOALTSTACK, OP_TOALTSTACK])
        )
        assert worked
        # Check stack
        stk = sm.stack()
        ...
        assert worked
        worked = sm.begin(
            CScript([OP_IF, OP_IF, OP_2, OP_ELSE, OP_3, OP_ENDIF, OP_ENDIF])
        )
        count = 0
        try:
            while 1:
                ...
                assert (
                    sm.pos() == count
                )  # only matches count because every opcode in the script is 1 byte (no pushdata)
        ...
        worked = sm2.eval(
            CScript([OP_IF, OP_IF, OP_3, OP_ELSE, OP_2, OP_ENDIF, OP_ENDIF])
        )
        assert worked
        stk = sm2.stack()
    ...
    def run_test(self):
        ...
        assert (
            ret.hex()
            == "c8e6c337c4fce20c6fc5861225591e1104c559c038fcf6f7429837f664209c7e"
        )
        ret = cashlib.txidem(tx)
        assert (
            ret.hex()
            == "df297c043efd84657387d675de57f8c8d69ac2290644aff12ac5ad66555a0980"
        )
        ...
        tx2 = CTransaction()
        tx2.vin.append(
            CTxIn(COutPoint().fromIdemAndIdx(txidem, 0), amt, b"", 0xFFFFFFFF)
        )
